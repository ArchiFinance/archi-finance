...
contract CollateralReward is AbstractReward {
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "CollateralReward: Caller is not the operator");
        _;
    }
    ...
    function stakeFor(address _recipient, uint256 _amountIn) public override nonReentrant onlyOperator {
        require(_amountIn > 0, "CollateralReward: Amount cannot be 0");
    ...
    function withdrawFor(address _recipient, uint256 _amountOut) public nonReentrant onlyOperator returns (uint256) {
        User storage user = users[_recipient];
    ...
    function withdraw(uint256 _amountOut) public override nonReentrant onlyOperator returns (uint256) {
        return withdrawFor(msg.sender, _amountOut);
    }
}
